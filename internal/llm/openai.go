...
func (d *OpenAIDecider) Decide(symbol string, latest types.Candle, inds types.Indicators, ctxmap map[string]any) (types.Decision, error) {
	...
	body := map[string]any{"model": d.cfg.LLM.Model, "messages": []map[string]string{{"role": "system", "content": d.cfg.LLM.System}, {"role": "user", "content": prompt}}, "temperature": d.cfg.LLM.Temperature, "max_tokens": d.cfg.LLM.MaxTokens}
	bb, _ := json.Marshal(body)
	req, _ := http.NewRequestWithContext(context.Background(), "POST", "https://api.openai.com/v1/chat/completions", bytes.NewReader(bb))
